 ...
 * The TanStack mark.
 *
 * react-icons 5.5.0 predates Simple Icons adding this glyph, and bumping the
 * dependency two minor versions to obtain one icon pulls in every other icon
 * change with it. react-icons is itself a wrapper around Simple Icons paths, so
 * this is the same CC0 artwork from the same source, just not gated on an
 * upgrade. Fetched from https://cdn.simpleicons.org/tanstack.
 *
 * `currentColor` is deliberate: the upstream file ships TanStack's cream brand
 * fill, which would ignore the tag's text colour and look broken in light mode.
 */
export default function TanstackIcon({
  size = 22,
  ...props
